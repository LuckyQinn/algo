...
static skiplist_node_t *skiplist_node_new(int key, int level)
{
	...
	skiplist_node_t *t = new skiplist_node_t;
	assert(t != NULL);
	t->key = key;
    if (level > 0) {
        t->next = new skiplist_node_t *[level];
        memset(t->next, 0, sizeof(skiplist_node_t *)*level);
    }
	return t;
}

static void skiplist_node_free(skiplist_node_t *n)
{
    if (n != NULL) {
        delete []n->next;
        delete n;
...
void skiplist_free(skiplist_t *l)
{
    if(l != NULL)
    {
        skiplist_node_free(l->head);
        skiplist_node_free(l->tail);
        delete l;
        l = NULL;
    }
}
